...
ExpressionResult loader(CppModule *module) {
	...
	module->addFunction("setPixelColor", {{"x", INT}, {"y", INT}, {"color", INT}}, NONE,
						[](RPNFunctionArgsValue &args, const TextRange &range,
						   ContextPtr context) -> RPNFunctionResult {
							...
							if (pixel < 40 || pixel > 49) {
								return ExpressionResult("Pixel color must be between 40 and 49",
														args[2]->getRange(), context);
							}
							std::cout << "\033[" << y << ";" << x << "H";
	...
	module->addFunction(
		"setRGBPixelColor", {{"x", INT}, {"y", INT}, {"red", INT}, {"blue", INT}, {"green", INT}},
		NONE,
		[](RPNFunctionArgsValue &args, const TextRange &range,
		   ContextPtr context) -> RPNFunctionResult {
			...
			if (red < 0 || red > 255 || blue < 0 || blue > 255 || green < 0 || green > 255) {
				return ExpressionResult(
					...
					red < 0 || red > 255
						? args[0]->getRange()
						: (blue < 0 || blue > 255 ? args[1]->getRange() : args[2]->getRange()),
					context);
			}
			std::cout << "\033[" << y << ";" << x << "H";
	...
	module->addFunction(
		"setAscii",
		{{"x", INT}, {"y", INT}, {"letter", STRING}, {"color", INT}, {"background", INT}}, NONE,
		[](RPNFunctionArgsValue &args, const TextRange &range,
		   ContextPtr context) -> RPNFunctionResult {
			...
			if (color < 30 || color > 39) {
				return ExpressionResult("Color must be between 30 and 39", args[3]->getRange(),
										context);
			}
			if (background < 40 || background > 49) {
				return ExpressionResult("Background must be between 40 and 49", args[4]->getRange(),
										context);
			}
			std::cout << "\033[" << y << ";" << x << "H";
	...
	module->addFunction(
		...
		[](RPNFunctionArgsValue &args, const TextRange &range,
		   ContextPtr context) -> RPNFunctionResult {
			...
			if (backgroundRed < 0 || backgroundRed > 255 || backgroundBlue < 0 ||
				backgroundBlue > 255 || backgroundGreen < 0 || backgroundGreen > 255) {
				return ExpressionResult("Pixel color must be between 0 and 255",
										backgroundRed < 0 || backgroundRed > 255
											? args[3]->getRange()
											: (backgroundBlue < 0 || backgroundBlue > 255
												   ? args[4]->getRange()
												   : args[5]->getRange()),
										context);
			}
			...
			std::cout << "\033[48;2;" << backgroundRed << ";" << backgroundGreen << ";"
					  << backgroundBlue << "m" << char(letter);
			return None::empty();
		});

	module->addFunction(
		"drawRect", {{"x", INT}, {"y", INT}, {"width", INT}, {"height", INT}, {"color", INT}}, NONE,
		[](RPNFunctionArgsValue &args, const TextRange &range,
		   ContextPtr context) -> RPNFunctionResult {
			int x = static_cast<Int *>(args[0])->getValue();
			int y = static_cast<Int *>(args[1])->getValue();
			...
			if (color < 40 || color > 49) {
				return ExpressionResult("Color must be between 40 and 49", args[4]->getRange(),
										context);
			}
			std::cout << "\033[" << y << ";" << x << "H";
	...
	module->addFunction(
		"drawText", {{"x", INT}, {"y", INT}, {"text", STRING}, {"color", INT}, {"background", INT}},
		NONE,
		[](RPNFunctionArgsValue &args, const TextRange &range,
		   ContextPtr context) -> RPNFunctionResult {
	...
	module->addFunction(
		...
		[](RPNFunctionArgsValue &args, const TextRange &range,
		   ContextPtr context) -> RPNFunctionResult {
			...
			std::cout << "\033[38;2;" << red << ";" << green << ";" << blue << "m";
			std::cout << "\033[48;2;" << backgroundRed << ";" << backgroundGreen << ";"
					  << backgroundBlue << "m" << text;
			return None::empty();
		});

	/**
	 * @brief Allo input to be captured from the keyboard wihout echoing to the screen and without
	 * requiring the user to press enter
	 *
	 */
	module->addFunction("rawMode", {}, NONE,
						[](RPNFunctionArgsValue &args, const TextRange &range,
						   ContextPtr context) -> RPNFunctionResult {
							struct termios raw;
	 ...
	 *
	 */
	module->addFunction("normalMode", {}, NONE,
						[](RPNFunctionArgsValue &args, const TextRange &range,
						   ContextPtr context) -> RPNFunctionResult {
	...
	/**
	 * @brief get user input from the keyboard non-blocking (returns empty string if no input is
	 * available)
	 *
	 */
	module->addFunction("getKey", {}, STRING,
						[](RPNFunctionArgsValue &args, const TextRange &range,
						   ContextPtr context) -> RPNFunctionResult {
							char input;
							if (read(STDIN_FILENO, &input, 1) == 1) {
								return new String(std::string(1, input), TextRange(),
												  Value::INTERPRETER);
							}
